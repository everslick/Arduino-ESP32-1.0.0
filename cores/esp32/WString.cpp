...
void String::replace(const String& find, const String& repl) {
    if(len() == 0 || find.len() == 0)
        return;
    int diff = repl.len() - find.len();
    char *readFrom = wbuffer();
    char *foundAt;
    if(diff == 0) {
        while((foundAt = strstr(readFrom, find.buffer())) != NULL) {
            memmove(foundAt, repl.buffer(), repl.len());
        ...
        while((foundAt = strstr(readFrom, find.buffer())) != NULL) {
            ...
            writeTo += n;
            memmove(writeTo, repl.buffer(), repl.len());
            writeTo += repl.len();
        ...
        while(index >= 0 && (index = lastIndexOf(find, index)) >= 0) {
            ...
            int newLen = len() + diff;
            memmove(wbuffer() + index, repl.buffer(), repl.len());
            setLen(newLen);
            wbuffer()[newLen] = 0;
            index--;
        }
...
void String::replace(const char * find, const String& repl) {
    replace(String(find), repl);
}
void String::replace(const __FlashStringHelper * find, const String& repl) {
    replace(String(find), repl);
}
void String::replace(const char * find, const char * repl) {
    replace(String(find), String(repl));
}
void String::replace(const __FlashStringHelper * find, const char * repl) {
    replace(String(find), String(repl));
}
void String::replace(const __FlashStringHelper * find, const __FlashStringHelper * repl) {
    replace(String(find), String(repl));
}
